...
#
#==============================================================================
# ".S_("This perl source file was automatically generated by")." $class
# ".S_("from Glade file")." $project->{'glade_filename'}
# ".S_("Date")." $project->{'date'}
#
# $class       - ".S_("version")." $VERSION
# ".S_("Copyright")." (c) ".S_("Date")." $DATE
# ".S_("Author")." $AUTHOR
#==============================================================================
...
${indent}__PACKAGE__->show_skeleton_message(\$me, \\\@_, ".
    "__PACKAGE__, \"\$Glade::PerlRun::pixmaps_directory/$Glade_Perl->{'logo'}\");
...
#        next if $form =~ /^__/;
        ...
        push @code, $class->perl_AUTOLOAD_top(
            $Glade_Perl, $proto, $form, $permitted_stubs)."\n";
#print "Working on form '$form'\n";
#use Data::Dumper; print Dumper($forms);
#print "'",join("'\n", @{$forms->{$form}{'UI_Strings'}}),"'\n";
        $UI_String = join("\n", @{$forms->{$form}{'UI_Strings'}});
        push @code, $UI_String;
        push @code, $class->perl_constructor_bottom($Glade_Perl, $form);
        push @code, "\n\n\n\n\n\n\n\n";
    }
    push @code, $class->perl_doc($Glade_Perl, $proto->{'UI_class'}, $first_form);
...
${indent}# ".S_("installed in the normal place")." (eg /usr/local/share/locale/".
    $main::Glade_Perl_Generate_options->source_LANG."/LC_MESSAGES/".$project->{'name'}.".mo)
...
${indent}\$Glade::PerlRun::pixmaps_directory ||= '$Glade_Perl->{'glade_proto'}{'project'}{'pixmaps_directory'}';
...
${indent}${indent}or die \"\$self \"._(\"is not an object so we cannot\").\" '\$AUTOLOAD'\\n\",
${indent}${indent}${indent}_(\"We were called from \"),join(\", \", caller),\"\\n\\n\";
${indent}my \$name = \$AUTOLOAD;
${indent}\$name =~ s/.*://;       # ".S_("strip fully-qualified portion")."

${indent}if (exists \$self->{_permitted_fields}->{\$name} ) {
${indent}${indent}# ".S_("This allows dynamic data methods - see \%fields above")."
...
${indent}${indent}# ".S_("This shows dynamic signal handler stub message_box - see \%stubs above")."
...
${indent}${indent}
${indent}} else {
${indent}${indent}die _(\"Can't access method\").\" `\$name' \"._(\"in class\").\" \$type\\n\",
${indent}${indent}${indent}_(\"We were called from\").\" \",join(\", \", caller),\"\\n\\n\";
...
# ".S_("into the relevant classes in your application or its subclasses")."\n";
    ...
    print SIGS $class->perl_doc(
        $Glade_Perl, $proto->{'SIGS_class'}, $first_form);
...
# ".S_("This is the basis of an application with signal handlers")."\n";
        ...
        print SIGS join("\n", @code);
        print SIGS $class->perl_doc(
            $Glade_Perl, $proto->{'APP_class'}, $first_form);
    }
}
...
#==============================================================================";
    ...
    push @code, $class->perl_doc(
        $Glade_Perl, $proto->{'SUBAPP_class'}, "Sub".$first_form);
...
#==============================================================================";
    ...
    push @code, $class->perl_doc(
        $Glade_Perl, $proto->{'LIBGLADE_class'}, $proto->{'LIBGLADE_class'});
